...
/// <summary>
/// Fetches guild metadata + channel list from Discord, publishes ChannelSyncDue
    ...
    ILogger<GuildSyncConsumer> logger) : IConsumer<GuildSyncDue>
{
    ...
    public async Task Consume(ConsumeContext<GuildSyncDue> context)
    {
        ...
        try
        {
            await ConsumeCore(context, guildId, ct);
        }
        catch (HttpRequestException ex) when (
            ex.StatusCode is HttpStatusCode.Forbidden or HttpStatusCode.NotFound)
        {
            logger.LogWarning(
                "Guild {GuildId}: Discord returned {StatusCode} — marking not present",
                guildId, ex.StatusCode);
    ...
    private async Task ConsumeCore(ConsumeContext<GuildSyncDue> context, long guildId, CancellationToken ct)
    {
        ...
        // Batch-fetch existing cursors so each ChannelSyncDue resumes from its last
        ...
        foreach (var channel in allChannels)
        {
            ...
            await context.Publish<ChannelSyncDue>(new
            {
